...
""",
    ...
    for text, answer in zip(inputs, answers):
        ...
        A = [None] * H
        for i in range(H):
            A[i] = list(map(int, text.splitlines()[i + 1].split()))

        B = [None] * H
        for i in range(H):
            B[i] = list(map(int, answer.splitlines()[i].split()))
        assert main(H, W, A) == B


def main(H, W, A):
    rowsum = [sum(row) for row in A]
    colsum = get_colsum(H, W, A)
